...
const parseMaterialyInfografiky = async (infografiky: Infografika[] | undefined) => {
  ...
  const parsedInfografiky = infografiky.map(async (infografika) => {
    return {
      ...infografika,
      materialy: {
        nahledy: await Promise.all(infografika.materialy.nahledy.map(async (nahled) => {
          return {
            ...nahled,
            nahled: await parseAsset(nahled.nahled)
          }})),
        soubory: await Promise.all(infografika.materialy.soubory.map(async (soubor) => {
          return {
            ...soubor,
...
const parsetMaterialyEbook = async (ebooky: Ebook[] | undefined) => {
  ...
  return await Promise.all(ebooky.map(async (ebook) => {
...
const fetchData = async () => {
  ...
  materialyInfografiky.value = await parseMaterialyInfografiky(proInstituceAcf.value?.materialy?.infografiky)
...
<template>
  <section v-if="proInstituceAcf" class="max-w-[900px]">
    ...
    <section class="flex flex-col items-center p-8 md:p-16 gap-8 md:gap-16 md:w-full box-border">
      ...
      <div
          v-if="materialyInfografiky"
          class="flex flex-col gap-8 py-8 md:gap-16"
      >
        <h3
          class="text-20-26 md:text-30 text-primary font-baloo font-semibold text-center m-0"
        >
        ...
        >
          <h4
            class="text-17 md:text-heading text-orange font-baloo font-semibold m-0"
          >
            {{infografika.nadpis}}
          </h4>
          ...
          <div
            class="flex flex-col md:flex-row md:justify-center items-center md:gap-8"
          >
            <img
