...
onMounted(async() => {
  ...
  if(router.currentRoute.value.query) {
    const {data} = await axios.post(`${import.meta.env.VITE_BASE_URL}/wp-json/draftspot_theme/v1/payment_status/`, {
  ...
  await fetchData()
  const response = await axios.post(`${import.meta.env.VITE_BASE_URL}/wp-json/draftspot_theme/v1/confirmation/`,
      {
...
const waitingForPayment = computed(() => {
  return paymentData.value?.state === 'CREATED'
...
<template>
<section v-if="paymentData" class="flex flex-col items-center justify-center gap-12 px-8 py-6 max-w-[900px]">
  <h1
  ...
  >
    {{ isPaid ? paymentDetails?.zaplaceno.nadpis : paymentDetails?.nezaplaceno.nadpis}}
  </h1>

  <p
      v-html="isPaid ? paymentDetails?.zaplaceno.popis : paymentDetails?.nezaplaceno.popis"
      class="text-16 font-roboto text-text-gray m-0"
  />

  <section v-if="isPaid" class="flex flex-col shadow-warning-sign w-full py-8 text-text-gray">
    <h2 class="text-primary font-baloo text-clamp text-center">Shrnutí objednávky č. {{orderData?.orderId}}</h2>
    <div class="flex flex-col p-8 md:p-12 gap-12">
      <article class="flex flex-col gap-4">
        <h3 class="text-20 m-0 font-roboto">Zakoupené produkty:</h3>

        <div class="flex gap-4 flex-nowrap text-base font-roboto text-text-gray justify-between font-normal">
          <span>{{orderData?.mistastrachu}} x</span>
          <span>Místa strachu</span>
          <span> {{orderData?.total ? (parseInt(orderData.total) as number) / orderData.mistastrachu : ''}} Kč/kus</span>
      ...
      <article class="flex flex-col gap-2">
        ...
        <span class="text-base font-roboto text-text-gray font-normal">{{`${orderData?.name}, ${orderData?.address}`}}</span>
      </article>
  ...
  >
    ...
    <span class="text-16 text-text-gray text-center py-4">Případně můžete využít tlačítko níže pro opakovanou platbu.</span>
  ...
  <section v-else class="flex flex-col items-center justify-center shadow-warning-sign w-full py-12 text-text-gray">
    <h2 class="text-primary font-baloo text-clamp text-center">Zkuste prosím provést platbu znova</h2>
  ...
  <section>
    <div v-if="paymentDetails?.dotazy" class="flex flex-col items-center justify-center gap-6">
      <h2 class="text-primary text-clamp text-center font-baloo m-0">{{ paymentDetails.dotazy.nadpis }}</h2>
